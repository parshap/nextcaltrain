...
var getStopStation = require("./util").getStopStation;
...
module.exports = function(store, scheduledStop, fromStopId, toStopId) {
  var stopTimes = getStopTimesFromTrip(store, scheduledStop.stopTime.trip_id);
  var startStopTime = findStopTimeAtStop(store, stopTimes, fromStopId);
  var endStopTime = findStopTimeAtStop(store, stopTimes, toStopId);
  if ( ! startStopTime) {
  ...
  return stopTimes.map(function(stopTime) {
    var station = getStopStation(store, stopTime.stop_id);
    return {
      date: setDateTime(cloneDate(scheduledStop.serviceDate), stopTime.arrival_time),
      stopTime: stopTime,
      stop: store.stops[stopTime.stop_id],
      station: station,
    };
  });
};
